package com.msf.macro;

import com.msf.config.MacroConfig;
import com.msf.feature.AntiAfk;
import com.msf.feature.BiteDetector;
import com.msf.feature.ChatSeaCreatureDetector;
import com.msf.feature.FailsafeHandler;
import com.msf.feature.SeaCreatureDetector;
...
public class FishingMacro {
    ...
    private void handleWaitingForBite() {
        ...
        // Track bobber position for proximity-based entity filtering
        if (mc.player.fishHook != null) {
            lastBobberPos = mc.player.fishHook.getEntityPos();
        }

        // Check for sea creatures near the bobber (only if kill is enabled)
        if (MacroConfig.killSeaCreatures) {
            Optional<LivingEntity> creature = seaCreatureDetector.detectSeaCreature(lastBobberPos);
            if (creature.isPresent()) {
                targetCreature = creature.get();
                System.out.println("[MSF] Creature detected: " + targetCreature.getType()
    ...
    private void handleReCasting() {
        ...
        // Check if a sea creature is near where the bobber was (only if kill is enabled)
        if (MacroConfig.killSeaCreatures) {
            Optional<LivingEntity> creature = seaCreatureDetector.detectSeaCreature(lastBobberPos);
            if (creature.isPresent()) {
                targetCreature = creature.get();
    ...
    private void handleKilling() {
        ...
        if (killTimeout.passed()) {
            if (targetCreature != null) {
                System.out.println("[MSF] Kill timeout for " + targetCreature.getType()
    ...
    private void handleMeleeKill() {
        ...
        if (dist > 9.0) { // > 3 blocks
            ...
            if (Math.random() < 0.05) {
                com.msf.mixin.KeyBindingAccessor sprintAcc =
                        (com.msf.mixin.KeyBindingAccessor) mc.options.sprintKey;
    ...
    private void scanHotbarForItems() {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            System.out.println("[MSF] Slot " + i + ": " + itemName);
        ...
        System.out.println("[MSF] Detected rod slot: " + detectedRodSlot
                + ", weapon slot: " + detectedWeaponSlot);
    }

    private void changeState(MacroState newState) {
        if (state != newState) {
            System.out.println("[MSF] " + state + " -> " + newState);
            state = newState;
        }
    }
}
